...
class Clue:
    ...
    def syncProjects(self, workspace):
        ...
        for p in prjs:
            project_name = p["name"]
            self.logger.info("Adding project %s (%d of %d projects)" % (project_name, idx + 1, numPrjs))

            # Prepare Group assignment to Projects
            pgroups = self.toggl.getProjectGroups(project_name, workspace)
            ...
            if project_name not in clockifyPrjNames:
                err = False
                clientName = None
                if "cid" in p:
                ...
                billable = p["billable"]
                color = p["hex_color"]
                members = self.toggl.getProjectUsers(project_name, workspace)
                if members is None:
                    members = []
                ...
                for member in members:
                    ...
                    try:
                        ...
                        if member["manager"]:
                            manager = True
                        m.addMembership(userMail, project_name, workspace,
                                        membershipType="PROJECT", membershipStatus="ACTIVE",
                                        hourlyRate=None, manager=manager)
                    except Exception as e:
                        fallback_user_email = m.connector.fallbackUserMail
                        if fallback_user_email is not None:
                            self.logger.warning("user %s not found in clockify project, falling back to %s" %
                                                (userMail, fallback_user_email))
                            m.addMembership(fallback_user_email, project_name, workspace,
                                            membershipType="PROJECT", membershipStatus="ACTIVE",
                                            hourlyRate=None, manager=manager)
                        else:
                ...
                if not err:
                    ...
                    rv = self.clockify.addProject(project_name, clientName, workspace, isPublic, billable,
                    ...
                    if (rv == ClockifyAPI.RetVal.OK) and (pgroups != []):
                        ...
                        # try?
                        project_id = self.clockify.getProjectID(project_name, workspace)
                        self.clockify.addGroupsToProject(workspace, wsId, project_id, wgroupIds, pgroups)
                        self.logger.info(" ...ok, done.")
                        numOk += 1
                    elif rv == ClockifyAPI.RetVal.EXISTS:
                        self.logger.info("... project %s already exists, skip..." % project_name)
                        numSkips += 1
                        ...
                        self.logger.error(" Could not add project %s. %s was project admin in toggl, but seems to "
                                          "be no admin in clockify. Check your workspace settings and grant admin "
                                          "rights to %s." % (project_name, manager, manager))
                        sys.exit(1)
                    else:
                        numErr += 1
                else:
    ...
    def onNewReports(self, entries, totalCount):
        ...
        if entries == None and totalCount == 0:
            ...
            for e in entries:
                self._idx += 1
                self.logger.info(
                    "adding entry %s, project: %s (%d of %d)" % (e["description"], e["project"], self._idx, totalCount))
                ...
                try:
                    ...
                    try:
                        ...
                        if self._skipInvTogglUsers:
                            ...
                            continue
                        elif self.clockify.fallbackUserMail is not None:
                            userMail = self.clockify.fallbackUserMail
                            self.logger.info("user '%s' not found in clockify workspace, using fallback user '%s'" % (
                            userName, userMail))
                        else:
                            raise
